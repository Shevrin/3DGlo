const sendForm = ({ formId, someElement = [] }) => {
	...
	const loadText = `<?xml version="1.0" encoding="UTF-8" standalone="no"?>
	...
	const validate = (list) => {
		let success = true
		list.forEach(input => {
			if (input.matches('[name="user_name"]') && input.value.length < 2) {
				success = false
			}
			if (input.matches('[name="user_email"]') && input.value.length === 0) {
				success = false
			}
			if (input.matches('[name="user_phone"]') && input.value.length === 0) {
				success = false
			}
